...
/**
 * Extracts inline Arabic format from values: "Pizza [ar-ae]:بيتزا" → base="Pizza", [ar-ae]="بيتزا"
 * Handles the pattern where a column value contains both English and Arabic inline
 * @param row The data row to process
 * @returns The row with inline Arabic extracted to separate columns
 */
const extractInlineArabic = (row: any): any => {
  const result = { ...row };

  // Pattern: text [ar-ae]:arabic text
  const inlineArabicPattern = /^(.+?)\s*\[ar-ae\]:(.+)$/;

  Object.keys(result).forEach(key => {
    const value = result[key];
    if (!value || typeof value !== 'string') return;

    const match = value.match(inlineArabicPattern);
    if (match) {
      const [, englishPart, arabicPart] = match;

      // Update the current column to just the English part
      result[key] = englishPart.trim();

      // Create the [ar-ae] column with the Arabic part
      const arabicKey = `${key}[ar-ae]`;
      result[arabicKey] = arabicPart.trim();
    }
  });
...
export const transformMenuData = (
  ...
  for (let i = 0; i < rawData.length; i++) {
    ...
    // Extract inline Arabic format (e.g., "Pizza [ar-ae]:بيتزا" → "Pizza" + [ar-ae]:"بيتزا")
    const rowBeforeExtract = Object.keys(row);
    row = extractInlineArabic(row);
    // Track any new [ar-ae] columns generated by inline extraction
    const rowAfterExtract = Object.keys(row);
    rowAfterExtract.forEach(col => {
      if (!rowBeforeExtract.includes(col) && col.includes('[ar-ae]')) {
        generatedColumns.add(col);
      }
    });
